...
import com.ds.tools.data.generator.exception.DataGeneratorException;
...
public final class DataGenerationStarter {
    ...
    static {
        final String pattern = "%-5p: %c - %m%n";
        console.setLayout(new PatternLayout(pattern));
        console.setThreshold(Level.INFO);
        console.activateOptions();
        Logger.getLogger("com.ds.tools.data.generator")
              .addAppender(console);
    }
    ...
    /**
     * Psuedo main method to start data generation and export process for all the
     * CLI Queries passed. To get hold of generated data use
     * {@link DataGenerator#generate()} after instantiating {@link DataGenerator}
     * using factory method {@link DataGenerator#from(String[])}
     *
     * @param query
     *            Single CLI Query (may contain multiple queries in same String)
     */
    public static void start(final String query) {
        ...
        if (StringUtils.startsWith(query, Constants.JSON)) {
            ...
            final String[] jsonFiles = Arrays.stream(StringUtils.split(query))
                                             .filter(arg -> arg.endsWith(Constants.JSON_EXTENSTION))
                                             .toArray(size -> new String[size]);
            log.info("JSON files to generate data : {}", Arrays.toString(jsonFiles));
            cliQueries = StringUtils.split(JsonQueryBuilder.getInstance()
                                                           .build(Arrays.asList(jsonFiles)), Constants.CLI_QUERY_SEPARATOR);
        } else {
        ...
        Arrays.stream(cliQueries)
              .forEach(DataGenerationStarter::invokeDataGeneratorFor);
     ...
     * exports data (if specified) for single query. To get hold of generated data
     * use {@link DataGenerator#generate()}
     *
     * @param cliQuery
     *            for which data generator should run
     */
    public static void invokeDataGeneratorFor(final String cliQuery) {
        try {
            final long start = System.nanoTime();
             ...
             * interpreted and registered with query runners by DataGenerator.
             */
            ...
            final Collection<DataCollector> collectedData = DataGenerator.from(DataGeneratorUtils.getDataGenQueries(cliQuery))
                                                                         .generate();

            // check and export data
            if (DataGeneratorUtils.shouldExportToFile(cliQuery)) {
                DataExporter.from(DataGeneratorUtils.getDataExportQueries(cliQuery))
             ...
             * DataGeneratorUtils.checkAndOverrideQueryDefaults for this cliqQuery so next
             * cliQuery can use Data Generator Defaults in case it does not want to override
             * them. Must be restored after current query has executed.
             */
