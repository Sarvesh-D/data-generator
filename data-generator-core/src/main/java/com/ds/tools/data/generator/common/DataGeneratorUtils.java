...
public enum DataGeneratorUtils {
     ...
     */
    public static void checkAndOverrideDataGeneratorDefaults(final String cmdLineQuery) {
        ...
        if (cmdLineQuery.contains(Constants.GLOBAL_OVERRIDE)) {
            ...
            Interpreters.GLOBAL_DEFAULT_OVERRIDE.get()
                                                .doInterpret(null, StringUtils.substringAfter(cmdLineQuery, Constants.GLOBAL_OVERRIDE));
        }
    }

    /**
     * Extracts one or more dataExportQueries from the complete CLI Query passed.
     *
     * @param cmdLineQuery
     *            to extract data export queries from
     * @return array containing one or more dataExport queries.
     */
    public static String[] getDataExportQueries(final String cmdLineQuery) {
        final String completeDataExportQueryString = StringUtils.substringBetween(cmdLineQuery, Identifiers.DATA_EXPORT_QUERY_PREFIX.getIdentifier()
                                                                                                                                    .toString(), Identifiers.DATA_EXPORT_QUERY_SUFFIX.getIdentifier()
                                                                                                                                                                                     .toString());

        // gather all CMD queries to export data
        final String[] dataExportQueries = StringUtils.split(completeDataExportQueryString, Identifiers.QUERY_SEPARATOR.getIdentifier());
     ...
     */
    public static String[] getDataGenQueries(final String cmdLineQuery) {
        final String completeDataGenQueryString = StringUtils.substringBetween(cmdLineQuery, Identifiers.DATA_GEN_QUERY_PREFIX.getIdentifier()
                                                                                                                              .toString(), Identifiers.DATA_GEN_QUERY_SUFFIX.getIdentifier()
                                                                                                                                                                            .toString());

        // gather all CMD queries to generate data
        final String[] dataGenQueries = StringUtils.split(completeDataGenQueryString, Identifiers.QUERY_SEPARATOR.getIdentifier());
    ...
    public static void restoreDataGeneratorDefaults() {
        ...
        final String overrideIdentifiers = Identifiers.QUANTITY.getIdentifier() + String.valueOf(Defaults.DEFAULT_QUANTITY);
        Interpreters.GLOBAL_DEFAULT_OVERRIDE.get()
                                            .doInterpret(null, overrideIdentifiers);
     ...
     * @return <code>True</code> if data export flag is present, <code>False</code>
     *         otherwise.
     */
    public static boolean shouldExportToFile(final String cmdLineQuery) {
        // check query for any data exporters
        final boolean exportToFile = ArrayUtils.contains(StringUtils.split(cmdLineQuery), Identifiers.FILE.getIdentifier()
                                                                                                          .toString());
