...
class GlobalDefaultOverrideInterpreter implements Interpreter {
    ...
    /**
     * {@inheritDoc}. This method looks up for any values that must be overriden and
     * accordingly updates them. Each time this method is invoked the defaults of
     * data-generator will be overriden accordingly. This interpreters sole job is
     * to override few of the default settings. These default settings overriden by
     * this interpreter applies to all the queries, hence these should not be set
     * individually in the queryBuilder. These properties of corresponding
     * interpreter should be set/overriden instead.
     */
    @Override
    public void doInterpret(final QueryBuilder queryBuilder, final String overrideIdentifiers) {
        Assert.assertNull("QueryBuilder supplied to GlobalPropertyOverrideInterpreter should always be null", queryBuilder);

        final Optional<Integer> quantityOverride = Arrays.stream(StringUtils.split(overrideIdentifiers, Constants.SPACE))
                                                         .filter(i -> i.charAt(0) == Identifiers.QUANTITY.getIdentifier())
                                                         .map(i -> Integer.valueOf(i.substring(1)))
                                                         .findFirst();
        if (quantityOverride.isPresent()) {
            /*
             * Need to override directly in Interpreters since Interpreters are registered
             * as Singletons.
             */
