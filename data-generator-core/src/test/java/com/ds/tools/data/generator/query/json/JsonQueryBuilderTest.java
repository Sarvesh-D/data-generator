...
public class JsonQueryBuilderTest {
    ...
    public final void testExecutionFromJson() {
        final StringJoiner jsonQuery = new StringJoiner(Constants.SPACE).add(Constants.JSON);
    ...
    public final void testExecutionFromMultipleJson() {
        ...
        jsonFilePaths.stream()
                     .forEach(jsonQuery::add);
        jsonQuery.add(Constants.DEBUG_ENABLED);
        DataGenerationStarter.start(jsonQuery.toString());

        final Path exportFile1 = Paths.get("sample_1.csv");
        assertNotNull("Path to where file was exported does not exists", exportFile1);

        final Path exportFile2 = Paths.get("sample_2.csv");
        assertNotNull("Path to where file was exported does not exists", exportFile2);

        final Path exportFile3 = Paths.get("sample_3.csv");
    ...
    public final void testInvalidJson() {
        JsonQueryBuilder.getInstance()
                        .build(Arrays.asList("src/test/resources/invalid.json"));
    }

    @Test
    public final void testValidJsonWExport() {
        final String cmdQuery_1 = JsonQueryBuilder.getInstance()
                                                  .build(Arrays.asList("src/test/resources/sample.json"));
        assertTrue("CMD query cannot be null or blank", StringUtils.isNotBlank(cmdQuery_1));
        final Collection<DataCollector> dataCollectedForQuery = DataGenerator.from(DataGeneratorUtils.getDataGenQueries(cmdQuery_1))
                                                                             .generate();
        assertTrue("Four data collector must be present", dataCollectedForQuery.size() == 5);

        DataExporter.from(DataGeneratorUtils.getDataExportQueries(cmdQuery_1))
